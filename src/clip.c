...
{// return square of dist...
 ...
 if(debug_var>=1) fprintf(errfp, "dist_from_rect(): x1,y1,x2,y2=%g,%g,%g,%g\n",x1,y1,x2,y2);
 if(debug_var>=1) fprintf(errfp, "dist_from_rect(): mx,my==%g,%g\n",mousex,mousey);
 if(debug_var>=1) fprintf(errfp, "dist_from_rect(): dist=%g\n",dist);
 return dist*dist;
}
...
{// return square of dist...
 ...
 yb = (a*a*ya - ab*xa - c*b) / denom;
     // debug ...
     if(debug_var>=1) fprintf(errfp, "dist(): dist1 = %g dist2 = %g\n",distance1,distance2);
 if(x1<x2)
 {
  if(xb >x1 && xb < x2)
  {
   ...
   distance3 = tmp*tmp / denom;
     // debug ...
     if(debug_var >=1) fprintf(errfp, "dist(); dist3 =  %g\n",distance3);
   return distance3;
  }
  else
  {
