...
#	05-11-2002  - Added u or U for signal setting: it is equal to z and Z (hi-Z), but
#		      in ncsim format z will force a 'z' to a signal, while u will release the signal.
#                    
#
# sample input:
#   time 0
...
/^[ \t]*s[ \t]+/{
 ...
 if(format=="relative_powermill" && time >=0)
 {
  ...
  {
     v=set_list[i]
     if(v ~ /^[10]$/) {vv=(v==1 ? voltage:0)}
     else if(v ~ /^[uUzZ]$/) {vv=halfvoltage}
     else {vv = v}
     if(v !~ /[uUzZ]/) print "force_node_v no=" i " v=" vv>file
  }
  print "\ncont " $2  unit "\n">file
  powmill_statement++
  delete set_list
...
/^[ \t]*set[ \t]+/{
 ...
 if($2 in buswidth)
 {
  ...
  for(i=1;i<=buswidth[$2];i++)
  {
   ...
   if(v ~ /[01zZuU]/) write_pwl_pair(s,v,$4)
  }
 }
 else 
 {
...
#                                                                \/
 ...
 else if(format=="relative_powermill")
 {
  ...
  for(i in set_list)
  {
     v=set_list[i]
     if(v ~ /^[10]$/) { vv=(v==1? voltage:0 ) }
     else if(v ~ /^[zZuU]$/) {vv=halfvoltage}
     else {vv = v}
     if(v !~ /[zZuU]/) print "force_node_v no=" i " v=" vv>file
  }
 }
 powmill_statement++
 break_lines(file)
...
function write_pwl_pair( name, value,res,   i,v,n,pwlres,pwltmpres,pwltmp2 )
{
  if(res=="") res=ron
  if(value ~ /^[10]$/) {v= ( value==1 ? voltage : 0 )  ; pwlres=res}
  else if(value ~ /^[zZuU]$/) {v=halfvoltage; pwlres=roff}
  ...
  if(!(name in signalname) ) 
  {
   ...
   signalname[name]="V" namex " XX" namex " 0 PWL " 
   simplesignalname[name]="V" namex " " name " 0 PWL " 
  ...
  if(signal[name,"value", signal[name,"n"] ]!=v)
  {
   timex=time < 0 ? 0: time
   if(timex==0)
   {
...
function break_lines(filename,    lines, line, l, i, count, style )
{
 lines=0
 style="+"
 close(filename)
